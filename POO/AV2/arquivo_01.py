...
class Abastecimento:
    ...
    def resumo(self):
        tipo_combustivel = (
            f"Gasolina {'Aditivada' if self.bomba.combustivel.aditivada else 'Comum'}"
            if isinstance(self.bomba.combustivel, Gasolina)
            else f"Etanol de {self.bomba.combustivel.origem}"
        )
        return f"Bomba {self.bomba.id_bomba} ({tipo_combustivel}): {self.qtd_litros} litros -> R$ {self.valor_total:.2f}"
...
class PostoDeCombustivel:
    ...
    def listar_bombas(self):
        return [f"Bomba {b.id_bomba}" for b in self.bombas]
